...
for (d in datasets) {
  ...
  while( is.null(cv.parest) ) {
    try(
  cv.parest <- cv4abc(parameters, sust, nval=100, tol =c(.05,.01,.005), transf=c("log","log","log","log","log"), prior.range = rbind(c(10000000,5000000000),c(1e-2,1.0),c(5,1000.0),c(150,900),c(8,900)), method = "loclinear")
    )
  }  
  ...
  for (i in 1:nrow(sust)) {
    ...
    post.parest <- abc(
      ...
      sumstat = sust[-i, ], 
      tol = .05,
      transf=c("log","log","log","log","log"), prior.range = rbind(c(10000000,5000000000),c(1e-2,1.0),c(5,1000.0),c(150,900),c(8,900)), method = "loclinear"
    )
  ...
  # Save to output file.
  write.csv(results,paste("LocLinear_CI_results_eta_",names(datasets[counter]),".csv",sep=""))
}